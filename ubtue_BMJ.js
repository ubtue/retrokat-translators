{
	"translatorID": "1815d08f-60c3-4a8e-9dc8-2aae1dd5b014",
	"label": "ubtue_BMJ",
	"creator": "Helena Nebel",
	"target": "bmj.com/content/",
	"minVersion": "2.1.9",
	"maxVersion": "",
	"priority": 98,
	"inRepository": true,
	...
	"lastUpdated": "2022-07-14 08:31:43"
}
	...
	ZU.doGet(risURL, function(risData) {
		if (risData.indexOf("<") == 0) {
			...
			trans.setHandler('itemDone', function (obj, item) {
				...
				let orcidAuthors = [];
				for (let orcidAuthor of ZU.xpath(doc, '//meta[(@name="citation_author") or (@name="citation_author_orcid")]/@content')) {
					orcidAuthors.push(orcidAuthor.textContent);
				}
				for (i=0; i<orcidAuthors.length-1; i++) {
					if (orcidAuthors[i+1].match('orcid.org/')) {
						item.notes.push( {note: "orcid:" + orcidAuthors[i+1].replace(/https?:\/\/orcid.org\//, '') + ' | ' + orcidAuthors[i]});
					}
				}
				...
				if ((ZU.xpathText(doc, '//span[contains(@class,"access-icon")]') == 'Free') || (ZU.xpathText(doc, '//meta[@name="DC.AccessRights" and contains(@content, open-access)]/@content') == 'open-access')) {
					item.notes.push('LF:')
				}
