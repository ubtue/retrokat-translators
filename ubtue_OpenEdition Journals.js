{
	"translatorID": "2c5eb32f-ba13-4f56-a231-62eb8077cb44",
	...
	"lastUpdated": "2022-03-08 10:07:49"
}
...
function scrape(doc, url) {
	...
	if (/persee-\d+/.test(url)) {
		...
		translator.setHandler('itemDone', function (obj, item) {
			if (ZU.xpathText(doc, '//div[@id="docSubParent"]//span[@class="title"]') != null) {
				if (ZU.xpathText(doc, '//div[@id="docSubParent"]//span[@class="title"]').match(/(Recensions)|(Bibliographie)/) != null) {
					item.tags.push("Book Review");
					if (ZU.xpathText(doc, '//p[@class="noticebibliooeuvre"]') != null) {
						let reviewed_title = ZU.xpathText(doc, '//p[@class="noticebibliooeuvre"]');
						Z.debug(reviewed_title);
						let isbn = reviewed_title.match(/ISBN\s*:?\s*([\d‑]+)/);
						Z.debug(isbn);
						if (isbn != null) {
