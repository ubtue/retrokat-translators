{
	"translatorID": "744cb334-7175-4fb3-b19a-d2539dca4231",
	"label": "ubtue_Dialnet",
	...
	"lastUpdated": "2022-07-14 12:16:51"
}
...
function scrape(doc, url) {
	...
	translator.setHandler('itemDone', function (obj, item) {
		...
		if (ZU.xpath(doc, '//meta[@name="DC.creator"]/@content').length == item.creators.length) {
			item.creators = [];
			for (let c of ZU.xpath(doc, '//meta[@name="DC.creator"]/@content')) {
				item.creators.push(ZU.cleanAuthor(c.textContent, 'author', true));
			}
		}
		...
		if (ZU.xpathText(doc, '//a[contains(@href,".pdf")]') != null) {
			if (ZU.xpathText(doc, '//a[contains(@href,".pdf")]').match('Texto completo') != null) item.notes.push('LF:');
		}
		item.complete();
	});
