{
	...
	"lastUpdated": "2025-05-21 08:45:50"
}
...
function getSearchResults(doc) {
  ...
  for (let i = 0; i < rows.length; i++) {
    let row = rows[i].textContent; // use textContent instead of .innerHTML to get plain text
    if (row.trim() !== "") {
      // only process non-empty rows
      let title = row;
      // Assume `title` is the entire text inside <a> -> adjust this as per your requirement
      let href = "";
      let hrefMatch = rows[i].innerHTML.match(/<a class="fa noul" href="([^\s"]+)/);
      if (hrefMatch) {
        href = hrefMatch[1];
        found = true;
        items[href] = title;
